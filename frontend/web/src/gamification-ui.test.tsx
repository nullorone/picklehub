...
describe('web gamification interface', () => {
    afterEach(() => {
        cleanup();
        vi.unstubAllGlobals();
    });
    ...
    it('shows accessible level progress, reasons and an explained reversal apart from sports data', async () => {
        ...
        render(
            <MemoryRouter>
                <ProgressScreen client={createIdentityClient({ baseUrl: '/v1' }, 'WEB')} />
            </MemoryRouter>
        );
            ...
            <MemoryRouter initialEntries={[`/progress/seasons/${seasonId}`]}>
                <Routes>
                    <Route
                        ...
                        element={<LeaderboardScreen client={createIdentityClient({ baseUrl: '/v1' }, 'WEB')} online />}
                    />
                </Routes>
            </MemoryRouter>
        );
