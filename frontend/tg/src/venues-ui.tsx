...
export function VenuesScreen({
    ...
    const searchList = useCallback(
        async (text: string, point: Origin | undefined) => {
            if (!online) {
                setMessage('Для обновления подключитесь к сети. Уже загруженный список остаётся доступен.');
                return;
            }
            setLoading(true);
            ...
            try {
                ...
                setMessage(
                    page.items.length === 0 ? 'Ничего не найдено. Измените фильтры, область или запрос.' : undefined
                );
            } catch (error) {
                setMessage(venueError(error));
            } finally {
