...
- Контракт `venues` принимает и выдаёт WGS84-координаты максимум с шестью десятичными знаками; большее число
  знаков отклоняется до записи. Radius ограничен 50 000 м, bbox — 100 км по каждой стороне без пересечения
  антимеридиана. Курсор живёт 15 минут и связан с режимом, полным набором фильтров и временем снимка.
- Расстояние вычисляет PostGIS в метрах с документированным оператором/index strategy. Клиентский расчёт не
  определяет eligibility или дедупликацию. Radius использует `ST_DWithin(indexed_geography, origin, metres)`, bbox
  — `ST_Intersects(indexed_geography, bounds)`; миграционные тесты обязаны подтверждать GiST plan на реалистичном
  наборе данных.
