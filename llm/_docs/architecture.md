...
- Backend остаётся модульным монолитом: один репозиторий и одна схема развёртывания, но явные границы модулей и
  владения данными.
- OpenAPI — источник истины для REST, AsyncAPI — для WebSocket и событий. Клиенты не определяют контракт по
  реализации backend.
...
- Редактируются только OpenAPI/AsyncAPI source documents и конфигурация генератора. Сгенерированные API clients,
  transport types и файлы Prisma имеют заголовок генерации и вручную не меняются.
