...
- Фактический disposition каждой текущей миграции, blocked rewrites и совместимость соседних application/worker/
  schema версий зафиксированы в
  [`production-readiness-contract-data.md`](production-readiness-contract-data.md). `BLOCKED` нельзя обходить
  увеличением timeout или незафиксированным maintenance window.
